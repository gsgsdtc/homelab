...
export function validateEnvironment(config: Record<string, unknown>): Record<string, unknown> & AppEnvironment {
  ...
  const modelProviderEncryptionKey = String(config.MODEL_PROVIDER_ENCRYPTION_KEY ?? "").trim();
  ...
  return {
    ...
    HOMELAB_WORKFLOW_MAX_SOURCE_BYTES: Number(config.HOMELAB_WORKFLOW_MAX_SOURCE_BYTES ?? 256 * 1024),
    HOMELAB_WORKFLOW_RUNTIME_URL: config.HOMELAB_WORKFLOW_RUNTIME_URL ? String(config.HOMELAB_WORKFLOW_RUNTIME_URL).trim() : undefined,
